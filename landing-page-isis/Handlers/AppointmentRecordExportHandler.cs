...
public class AppointmentRecordExportHandler(AppDbContext context) : IAppointmentRecordExportHandler
{
    ...
    private void ComposePdfHeader(IContainer container, Patient patient)
    {
        container.Column(col =>
        {
                    ...
                    row.RelativeItem()
                        .Column(c =>
                        {
                            c.Item()
                                ...
                                .FontSize(20)
                                .Bold()
                                .FontColor(Color.FromHex(SystemGreen));
            ...
            col.Item()
                .PaddingVertical(4)
                .LineHorizontal(1)
                .LineColor(Color.FromHex(SystemBurgundy));
        });
    }

    private void ComposePdfContent(
    ...
    )
    {
        container.Column(col =>
        {
            ...
            col.Item()
                .Text($"Histórico de Sessões ({records.Count})")
                .FontSize(14)
                .Bold()
                .FontColor(Color.FromHex(SystemBurgundy));
                    ...
                    .Column(recordCol =>
                    {
                        recordCol
                            ...
                            .Column(detail =>
                            {
                                var appointmentDate =
                                    record
                                        .Appointment?.AppointmentDate.ToPortoVelhoTime()
                                        .ToString("dd/MM/yyyy HH:mm")
                                    ?? "Data não informada";

                                detail
                                    .Item()
                                    .Text($"Sessão — {appointmentDate}")
                                    .FontSize(11)
                                    .Bold()
                                    .FontColor(Color.FromHex(SystemGreen));
    ...
    private byte[] GenerateDocx(Patient patient, List<AppointmentRecord> records)
    {
        ...
        )
        {
            ...
            AddDocxPara(body, $"Paciente: {patient.Name}", bold: true, size: 24, spacingAfter: 120);
            ...
            else
            {
                AddDocxPara(
                    body,
                    $"Histórico de Sessões ({records.Count})",
                    bold: true,
                    size: 28,
                    color: SystemBurgundy,
                    spacingAfter: 300
                );
            ...
            AddDocxParaCentered(
                body,
                "Gerado pelo Sistema Ísis — Dados confidenciais",
                size: 16,
                color: "808080",
                spacingAfter: 40
            );
            ...
            new OpenXml.DocDefaults(
                ...
                new OpenXml.ParagraphPropertiesDefault(
                    new OpenXml.ParagraphPropertiesBaseStyle(
                        new OpenXml.SpacingBetweenLines
                        {
                            After = "60",
                            Line = "276",
    ...
    private static void AddDocxPara(
        OpenXml.Body body,
        string text,
        bool bold = false,
        int size = 20,
    ...
    )
    {
        ...
        if (spacingBefore > 0)
            paraProps.AppendChild(
                new OpenXml.SpacingBetweenLines { Before = spacingBefore.ToString() }
            );

        if (spacingAfter > 0)
            paraProps.AppendChild(
                new OpenXml.SpacingBetweenLines { After = spacingAfter.ToString() }
            );
        ...
        var run = new OpenXml.Run(
            runProps,
            new OpenXml.Text(text)
            {
    ...
    private static void AddDocxParaCentered(
        OpenXml.Body body,
        string text,
        int size = 20,
        string? color = null,
