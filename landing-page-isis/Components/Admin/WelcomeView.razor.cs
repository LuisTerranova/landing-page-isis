    ...
    private async Task LoadAppointments()
    {
        ...
        try
        {
            ...
            if (_currentView == ViewMode.Day)
            {
                ...
                var endLocal = DateTime.SpecifyKind(date.AddDays(1).AddTicks(-1), DateTimeKind.Local);
            ...
            else
            {
                ...
                var monday = DateTime.SpecifyKind(_selectedDate.AddDays(-1 * diff).Date, DateTimeKind.Local);
                var endLocal = DateTime.SpecifyKind(monday.AddDays(7).AddTicks(-1), DateTimeKind.Local);
